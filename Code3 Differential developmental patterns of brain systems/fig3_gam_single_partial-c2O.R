...
name.colname <- c("MI_c2O","c2O_wholemask") # The indicators name in the data file
name.figshow <- c("mature_c2O")                               # The indicators name will present in the figure
con.col      <- c("darkolivegreen3")                         # The indicators color will present in the figure

### Make figure ###
for (i in 1:length(name.figshow)) {
  # Convert the read data into a format recognized by the package
  data.gam <- data.frame(data.fig[c("Group","Age1","Age2","Gender",name.colname[i],name.colname[i+1])])
  colnames(data.gam) <- c("Group","Age1","Age2","Gender","index.fig","index.fig.mean")
  
  # Select Smoothing Parameters with REML, Using P-splines and Draw by visreg and ggplot2
  fit.agedev <- gam(index.fig ~ s(Age1)+Gender+index.fig.mean, data=data.gam, method="REML")
  ...
  fig.agedev <- visreg(fit.agedev,                                     # The fitted model object you wish to visualize
    ...
    #annotate("text", x = 11.5, y = -0.30, label = paste("p<", p.v, sep = ""), size = 6) + 
  ...
  # Name of the figure
  fig.name <- paste("fig2_partial_",name.figshow[i], "_", p.v,".", fig.fmt, sep = "")
  # Save figure to disk
