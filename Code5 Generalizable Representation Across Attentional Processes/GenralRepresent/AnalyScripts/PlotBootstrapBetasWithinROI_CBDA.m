...
load('/home/haolei/Data/BrainDev_ANT/GenRep/Results/bs_CBDA_NeuroSynth_5000_age.mat');
...
for r=1:length(region_labels) % for each region
    ...
    figure;
    distributionPlot(squeeze(b(:,r,50:52)),'color',{[1 0 0] [0 1 0] [0 0 1]},'showMM',5); %plot bootstrap distributions for domain-level generalization indices
...
for r=1:length(region_labels) % for each region
    % subplot(1,14,r) %plot all on same figure
    figure;
    distributionPlot(squeeze(b(:,r,2:37)),'showMM',5); %plot bootstrap distributions for domain-level generalization indices
