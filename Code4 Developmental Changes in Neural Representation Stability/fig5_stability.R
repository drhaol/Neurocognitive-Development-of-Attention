...
fig.dpi <- 300; fig.wid <- 20; fig.hei <- 10; fig.fmt <- "eps" # Set parameters of the figure

setwd("D:/Documents/BaiduSyncdisk/Research/2018_Hao_AttenNeuroDev/Codes/DissocDegr")                             # Set the working directory
condname.colname <- c("r1_Grp_CBD_Comm_fdr05_r00_all","r2_Grp_CBD_Comm_fdr05_r01_fef_b","r3_Grp_CBD_Comm_fdr05_r02_spl_b",
                      "r4_Grp_CBD_Speci_fdr05_r01_ai_b","r5_Grp_CBD_Speci_fdr05_r02_dacc_r","r6_Grp_CBD_Speci_fdr05_r03_tpj_r")
condname.figshow <- c("r1_FEF+SPL","r2_FEF","r3_SPL","r4_Angular","r5_TPJ-STG","r6_dACC")

data.grp    <- "CBD"                     # Group of the data
data.subgrp <- c("g1CBDC", "g2CBDA")         # Subgroups of the data
g1CBDC <- read.csv("stability_g1C.csv") # Read result file of basic information (.csv) for subgroup 1
g2CBDA <- read.csv("stability_g2A.csv") # Read result file of basic information (.csv) for subgroup 2
...
fig.out <- ggplot(data = data.fig, 
                  aes(x = Index, y = Index.Data)) +
  geom_violinhalf(width = 0.4,aes(color=Group,fill=Group),position = position_nudge(x =.25, y = 0)) +
  #see::geom_violinhalf(width = 2, position = position_dodge(1.2))+
  geom_jitter(size=0.8,aes(color=Group),position = position_jitter(0.06))+
  geom_boxplot(width = 0.12,linewidth=0.8,aes(color=Group),position = position_nudge(x = .25, y = 0))+
...
fig.out
# Name of the figure
fig.name <- paste("fig_stability.", fig.fmt, sep = "")
# Save figure to disk
